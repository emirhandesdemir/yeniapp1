...
import { Loader2, Gem, UserCog as AdminUserCogIcon, ShieldAlert, Star, Users, Ban, CheckCircle, AlertOctagon, ShieldCheck, ShieldX, Trash2 } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { deleteUserAndTheirContent } from "@/lib/firestoreUtils";
...
export default function AdminUsersContent() {
  ...
  const handleDeleteUser = useCallback(async () => {
    ...
    setProcessingUserId(selectedUser.uid);
    setProcessingAction(true);
    try {
        await deleteUserAndTheirContent(selectedUser.uid);
        toast({ title: "Başarıyla Silindi", description: `${selectedUser.displayName || 'Kullanıcı'} ve tüm verileri sistemden kaldırıldı.` });
        fetchUsers();
        setIsDeleteUserDialogOpen(false);
        setSelectedUser(null);
    } catch (error) {
        console.error("Error deleting user:", error);
        toast({ title: "Silme Hatası", description: "Kullanıcı ve verileri silinirken bir hata oluştu.", variant: "destructive" });
    } finally {
                  ...
                  {users.map((user) => (
                    <TableRow key={user.uid} className={user.isBanned ? 'bg-destructive/10 hover:bg-destructive/20' : ''}>
                      ...
                      <TableCell className="text-right space-x-0.5 sm:space-x-1">
                        ...
                        <Button variant="destructive" size="icon" onClick={() => handleOpenDeleteDialog(user)} aria-label="Kullanıcıyı Sil" className="h-7 w-7 sm:h-8 sm:w-8" disabled={user.uid === adminAuthUser?.uid || processingAction || processingUserId === user.uid}>
                          {processingUserId === user.uid ? <Loader2 className="h-4 w-4 animate-spin"/> : <Trash2 className="h-3.5 w-3.5 sm:h-4 sm:w-4" />}
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      <Dialog open={isDeleteUserDialogOpen} onOpenChange={setIsDeleteUserDialogOpen}>
        <DialogContent className="sm:max-w-md">
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    <span className="font-bold text-destructive">Bu işlem geri alınamaz.</span>
                    <br />
                    <span className="font-medium text-foreground">{selectedUser?.displayName || "Kullanıcı"}</span> adlı kullanıcıyı ve bu kullanıcıya ait olan tüm gönderileri, yorumları ve oluşturduğu sohbet odalarını kalıcı olarak silmek istediğinizden emin misiniz?
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                <AlertDialogCancel disabled={processingAction}>İptal</AlertDialogCancel>
                <AlertDialogAction onClick={handleDeleteUser} disabled={processingAction} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground">
                    {processingAction && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                    Evet, Kullanıcıyı ve Tüm İçeriklerini Sil
                </AlertDialogAction>
            </AlertDialogFooter>
        </DialogContent>
      </Dialog>
