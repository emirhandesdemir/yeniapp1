...
export default function MatchPage() {
  ...
  useEffect(() => {
    ...
    // It's critical that it uses the .current property of refs inside the cleanup function
    // to get the LATEST state, not the state from when the effect was mounted.
    return () => {
      console.log(`[MatchPage Unmount/Cleanup] User: ${currentUser?.uid}, Current queueDocId: ${queueDocIdRef.current}, Current searchState: ${searchStateRef.current}`);
      
      if (unsubscribeQueueListenerRef.current) {
        console.log(`[MatchPage Unmount/Cleanup] Unsubscribing listener for ${currentUser?.uid}`);
        unsubscribeQueueListenerRef.current();
        unsubscribeQueueListenerRef.current = null;
      }
      
      // FIX: Use refs to get the latest state on cleanup.
      // This prevents deleting a doc for a user who has already been matched or is navigating away.
      if (queueDocIdRef.current && (searchStateRef.current === 'searching' || searchStateRef.current === 'idle')) {
        console.log(`[MatchPage Unmount/Cleanup] Cleaning up queue document ${queueDocIdRef.current} for user ${currentUser?.uid} as state was ${searchStateRef.current}.`);
        deleteDoc(doc(db, "matchmakingQueue", queueDocIdRef.current))
          .catch(e => console.warn("[MatchPage Unmount/Cleanup] Error cleaning up queue on unmount (best effort):", e));
      } else {
         console.log(`[MatchPage Unmount/Cleanup] No cleanup needed for queue document. State is ${searchStateRef.current}.`);
      }
    };
  }, [currentUser?.uid]); // Dependency ensures cleanup logic has access to the correct user context.
  ...
  const handleStartSearch = useCallback(async () => {
    ...
    try {
      ...
      if (matchedPartner && matchedPartnerDocRef && currentAttemptQueueId) {
        ...
        console.log(`[MatchPage handleStartSearch - Initiator ${currentUser.uid}] Deleting own queue document: ${newQueueDocRef.id} before navigating.`);
        ...
        unsubscribeQueueListenerRef.current = onSnapshot(selfQueueDocRef, async (docSnap) => {
          ...
          if (data?.status === 'matched' && data.temporaryDmChatId && data.matchedWithUserId) {
            ...
            try {
              console.log(`[Match Listener - ${currentUser.uid}] Deleting own queue document: ${docSnap.id} before navigating.`);
