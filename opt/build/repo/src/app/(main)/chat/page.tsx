...
export default function ChatRoomsPage() {
  const [chatRooms, setChatRooms] = useState<ChatRoomFeedDisplayData[]>([]);
  ...
  const [editingRoomDetails, setEditingRoomDetails] = useState<ChatRoomFeedDisplayData | null>(null);
  ...
  const fetchRooms = useCallback(async () => {
    ...
    const unsubscribe = onSnapshot(q, async (querySnapshot) => {
        ...
        try {
            const resolvedRooms = (await Promise.all(roomsPromises)).filter(room => room !== null) as ChatRoomFeedDisplayData[];
